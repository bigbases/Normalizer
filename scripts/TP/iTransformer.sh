...
if [ ! -d "./logs/TP/iTransformer" ]; then
  mkdir ./logs/TP/iTransformer
fi
...
for pred_len in 96 192 336 720; do
  ...
  python -u run_longExp.py \
    ...
    --learning_rate 0.001 \
    --station_lr 0.0005 \
    --batch_size 16 >logs/TP/$model_name/traf_$pred_len.log
  done
...
for pred_len in 96 192 336 720; do
  ...
  python -u run_longExp.py \
    ...
    --period_len 12 \
    --station_lr 0.001 \
    --pd_model 128 >logs/TP/$model_name/wea_$pred_len.log
  done
