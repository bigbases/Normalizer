...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # 'original': (x - mean) / std (기본 DDN 동작)
        # 'only_mean': x - mean (스케일링 효과 제거)
        # 'only_std': x / std (추세 제거 효과 제거)
        self.norm_variant = getattr(configs, 'norm_variant', 'original')
        if self.norm_variant not in ['original', 'only_mean', 'only_std']:
            raise ValueError(f"Unknown norm_variant: {self.norm_variant}")
        ### END NEW ###
    ...
    def de_normalize(self, input, station_pred):
        if self.station_type == 'adaptive':
            ...
            ### NEW ###
            # 4. norm_variant에 맞춰 비정규화(역연산) 수행
            if self.norm_variant == 'original':
                # Original: (백본 예측) * std + mean
    ...
    def norm(self, x, predict=True):
        # 1. norm_sliding을 호출하여 (정규화된 x)와 (원본 m, s)를 받음
        norm_x, (seq_m, seq_s) = self.norm_func(x)
        
        if predict is True:
            # 2. MLP는 항상 원본 m, s를 기반으로 미래 m, s를 예측함 (통제 변인)
            mov_m, mov_s = self.mlp(seq_m, seq_s, x)
            
            if self.j > 0: # DWT 분기 (DWT 내부에서도 norm_func가 동일한 variant로 호출됨)
                ...
                # 3. 백본 입력(norm_x)과 예측 통계량(mov_m, mov_s) 반환
    ...
    def norm_sliding(self, x, kernel=None):
        ...
        ### NEW ###
        # 1. norm_variant에 따라 백본에 들어갈 x를 다르게 정규화
        if self.norm_variant == 'original':
            x_normalized = (x - m) / (s + self.epsilon)  # (x - m) / s
        elif self.norm_variant == 'only_mean':
            x_normalized = x - m  # (x - m)
        elif self.norm_variant == 'only_std':
            x_normalized = x / (s + self.epsilon)  # x / s
        ### END NEW ###
        
        # 2. 통계량 예측기(MLP)를 위해서는 항상 원본 (m, s)를 반환
